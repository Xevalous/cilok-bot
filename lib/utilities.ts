...
export function autoPath(format: string, filename?: string, useTemp = true): string {
	if (useTemp && !existsSync(global.config.tempDir.split('/')[1])) mkdirSync(global.config.tempDir.split('/')[1]);
...
export async function run(): Promise<void> {
	try {
		...
		global.command = new CommandHandler();

		for (const a of readdirSync('./lib/commands')) {
			await import(`./commands/${a}`);
		}

		for (const a of readdirSync('./lib/events')) {
			await import(`./events/${a}`);
		}
