...
module.exports = class WAconnection {
    ...
    async reply (mess, text) {
        return await this.send(mess, {
            text: format(text),
            quoted: mess
        });
    }

    async sendSticker (mess, buffer, options = {}) {
        const prep = options?.exiffile
            ? await this.prepareSticker(buffer, options.exiffile)
            : await this.prepareSticker(buffer);
        return this.send(mess, { sticker: prep }, ...options);
    }
    ...
    async getBuffer (path, fileName = '', autoExt = true) {
        ...
        if (fileName) {
            const filename = autoExt ? `${fileName}.${template.ext}` : fileName;
