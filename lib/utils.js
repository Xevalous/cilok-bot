...
module.exports = class Util {
    constructor () {
        ...
        Object.entries(util).forEach(([a, b]) => {
            this[a] = b;
        });
    }
    ...
    autoPath (format = '', filename = '', useTemp = true) {
        const basePath = useTemp ? './tmp/' : '';
        return `${basePath}${
            filename
                ? filename.includes('?>')
    ...
    async metadataMsg (client, msg) {
        const chatMeta = async (mess) => {
            ...
            mess.quotedMsg = mess.quotedMsg
                ? (client.chats[mess.key.remoteJid] &&
                      client.chats[mess.key.remoteJid].messages[
                          mess.quotedMsg.key.id
                      ]) ||
    ...
    async run () {
        try {
            ...
            global.cmd = new Command(global.client, global.config, global.util);
